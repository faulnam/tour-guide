...
<body class="bg-[#F8FAF9] text-[#1A2E26] font-sans antialiased min-h-screen flex flex-col justify-between selection:bg-accent selection:text-neutral-dark">
        ...
        <div class="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
            <div class="flex items-center space-x-6">
                <a href="{{ route('karyawan.dashboard') }}" class="flex items-center gap-2 font-bold text-lg tracking-wider uppercase text-primary font-sans">
                    <div class="w-7 h-7 rounded-lg bg-accent flex items-center justify-center text-primary-dark shadow-sm">
                ...
                <nav class="hidden md:flex items-center space-x-6 text-xs uppercase tracking-wider font-bold">
                    <a href="{{ url('/') }}" class="text-gray-400 hover:text-primary transition-colors">&larr; Beranda Utama</a>
                    ...
                    <a href="{{ route('karyawan.absensi.index') }}" class="hover:text-primary transition-colors {{ request()->routeIs('karyawan.absensi*') ? 'text-primary border-b-2 border-primary pb-1' : 'text-gray-500' }}">Absensi Kamera GPS</a>
    ...
    @if(auth()->check() && (auth()->user()->isDemo() || str_contains(auth()->user()->email, 'demo')))
        <div class="bg-amber-500 text-black px-6 py-2 text-xs font-medium border-b border-amber-600 flex items-center justify-between flex-wrap gap-2 shadow-sm">
