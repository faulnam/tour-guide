...
@section('title', 'Edit Admin User: ' . $user->name)
...
<div class="max-w-xl mx-auto space-y-6">
    ...
    <form action="{{ route('admin.users.update', $user) }}" method="POST" class="bg-neutral-900 border border-neutral-800 p-6 md:p-8 space-y-6">
        @csrf
        @method('PUT')
        ...
        <div>
            <label for="password" class="block text-[11px] uppercase tracking-wider font-semibold text-neutral-300 mb-2">
                Password <span class="text-neutral-500 font-normal lowercase">(leave empty to keep current password)</span>
            </label>
            <input type="password" 
                   id="password" 
                   name="password" 
        ...
        <div>
            ...
            <select id="role" name="role" required class="w-full bg-neutral-950 border border-neutral-800 text-white text-xs px-4 py-3 focus:outline-none focus:border-white">
                ...
                <option value="editor" {{ old('role', $user->role) === 'editor' ? 'selected' : '' }}>Editor (Content &amp; Portfolio Only)</option>
            </select>
        </div>
